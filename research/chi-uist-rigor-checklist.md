...
**How to use this:** work section by section before submission. Each item states the criterion, which venue(s) it matters most for, and a verdict against your stated design — a **formative study (N=20–24)** feeding into a **3-condition main study (N=36)**. Where I don't know a design detail (e.g., between- vs. within-subjects, what the 3 conditions actually manipulate), I've flagged it as an **[ASSUMPTION — confirm]** rather than guessing silently.

**Stated design as I understand it** (confirm/correct before using this checklist further):
- Formative: N=20–24 community members/stakeholders, likely interviews + workshop(s), needs-finding → design requirements (BoundarEase-style two-phase, or WeDesign/Value-Centered-Framing-scale recruitment).
...
**Our status: MISSING / WEAK.** No power analysis is mentioned. N=36 across 3 conditions (~12/cell) is exactly VizCrit's cell size, which our own lit-review critique explicitly calls underpowered for multi-DV comparisons. If our main study has more than 2–3 outcome measures (likely, given zoning-compliance + creativity + inclusion + rapport + confidence would all be relevant), 12/cell is thin.
...
**Our status: LIKELY PARTIAL.** You have a formative N=20–24 study, but nothing in what you've shared states numbered findings/design-goals that map onto specific tool features (voice command set, AI facilitation style, zoning-feedback surfacing).
...
**Action:** Pre-register (OSF or AsPredicted) the main N=36 study: primary hypotheses, primary vs. secondary DVs, planned statistical tests + correction method, stopping rule/sample-size justification, and the Arnstein-ladder coding procedure. Keep the formative N=20–24 study explicitly exploratory/unregistered — that's normal and doesn't need registration, but say so to make the contrast clear to reviewers.
...
| C5 | **Small-N expert/formative walkthrough is an acceptable substitute** for a full study if explicitly scoped as such (Spacetime's N=6, Belidor's "Olsen 2007" justification for no human study on a language/grammar contribution) | Useful cover for the formative N=20–24 study if some of it is closer to expert/system walkthrough than full deployment — but don't use this to justify skipping rigor on the *main* N=36 study |
