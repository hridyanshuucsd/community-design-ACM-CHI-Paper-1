...
- **Formative study:** N=20 to 24, semi-structured interviews + one co-design workshop, community members + 2 to 3 planning or redevelopment professionals, Noida, needs-finding → numbered design goals (BoundarEase/Graphologue-style traceability).
- **Main study:** Between-subjects, N=36 individuals organized into **12 design teams of 3** (triads), **4 teams per condition**, one ~2.5-hour session per team on one of **two real or realistically-modeled Noida/Greater Noida group housing society redevelopment sites** (site assignment balanced 2-teams-per-site-per-condition to prevent site confound). Between-subjects, not within, because a single design session has strong path-dependence (you can't "forget" the design you just built to do it again in a different condition) and because session length (~2.5 hrs) makes repeated-measures burdensome.
...
1. **Hook + stakes.** A concrete vignette: a builder redevelopment proposal for a group housing society, a stakeholder meeting, a community that can react but not author. Ground in Arnstein's Ladder, most digital civic tools sit at rungs 3 to 5 (inform/consult), per the Luo et al. 2025 review's 16.7% co-developer statistic.
...
### 4.1 Method
- Recruitment channels stated explicitly (my own personal community relationships and local contacts in Delhi NCR/Noida, not an existing institutional pipeline, plus snowball sampling from residents who have gone through builder redevelopment disputes, RWA negotiations, or Noida Authority stakeholder processes), with demographic table (age, tenure/homeownership, prior civic engagement, primary language: Hindi, English, or both) reported honestly against the legitimacy claim (rigor gap #7).
...
| F4: Distrust of AI-generated content presented as authoritative | "If a computer made that up, why should the Authority listen to it?" | D4: AI content must be provenance-marked; compliance checks must be rule-based, not self-graded LLM output | Independent rule-based building-bylaw checker (§5.2); watermarking of AI-suggested geometry |
| F5: Preference for spoken, plain-language interaction over reading dense bylaw text, especially among older participants and those more comfortable in Hindi than English | "I don't want to read the building bylaws. Just tell me what I can do." | D5: Voice-first interaction as primary (not secondary/accessibility) modality | Malleable voice grammar generated from constraint graph, resolving Hindi, English, or code switched utterances (§6) |
...
- Plot ingestion → building-bylaw/setback/FAR/height/overlay-condition constraint graph, implemented as a new Pascal "system" (per the existing dirty-node systems pattern) rather than a geometry-engine fork.
...
- Explicit design decision, directly justified against LLMR's documented Inspector-leniency problem: the compliance checker is a deterministic rules engine validated against real Noida Authority building-bylaw determinations (ground-truthed against sanctioned building plans and UP RERA project filings gathered directly during the formative study, since no single structured, publicly queryable permit dataset exists for the region the way HCD's does for California, a real methodological difference worth naming rather than glossing over), *not* an LLM asked to grade its own edits.
- **Technical benchmark (decoupled from the human study, UIST-style systems contribution):** held-out set of N configurations (e.g., 300 synthetic + real parcel/design configurations spanning height, setback, FAR, overlay-district edge cases) scored against expert planner determination.
  ...
  - *Null:* kappa in the 0.5 to 0.7 range concentrated in *common* (not edge) cases, would indicate the constraint graph itself is under-specified for ordinary building-bylaw determinations, a system-validity problem that would need fixing before any human study could proceed, and should be reported as a limitation on the entire compliance-feedback claim if it persists into the human study.
...
Directly extends the malleable/generative UI argument (Cao/Jiang/Xia; Gradual Generation) to voice: the set of *sayable, actionable* utterances at any moment is generated from the same live constraint graph and multi-user selection/gaze state driving the visual layer, not a fixed command grammar, avoiding MozArt's small fixed vocabulary and Khan & Tunçer's expert-biased elicited vocabulary. In this field site the malleable grammar additionally has to resolve intent across Hindi, English, and code switched utterances (sometimes mixed mid sentence), rather than across vocabulary within a single language alone, a harder and more novel version of the claim than a monolingual deployment would be.

### 6.2 Vocabulary elicitation methodology
- Re-run a Digital-Modeling-for-Everyone-style Wizard-of-Oz elicitation with **non-expert community members** (not CAD-literate professionals, unlike Khan & Tunçer), N≈15 to 18 drawn partly from the formative-study pool, thematically analyzed for selection strategies, axis-convention assumptions, and "implicit construction" phrasing. The elicitation prompts explicitly invite participants to speak in whichever of Hindi, English, or a mix of both feels natural, rather than defaulting to English-only collection, so the resulting vocabulary reflects how the target population actually talks.
...
- **Error taxonomy** (adapted from Gazeify Then Voiceify + WordsEye/Ulinski's module-level taxonomy): parsing failure / reference-resolution failure (ambiguous object) / constraint-conflict misexplanation / disambiguation-failure (system asks wrong clarifying question) / execution failure (correct interpretation, wrong geometry produced) / language-mixing resolution failure (utterance correctly transcribed but intent misresolved specifically because it mixed Hindi and English, reported as its own category rather than folded into generic parsing failure).
...
- Between-subjects, 12 triads (N=36), 4 triads/condition, 2 real/realistic group housing society redevelopment sites × 2 triads/site/condition.
...
- **H2 (A vs. B vs. C, legitimacy):** Single-item self-report, "I would trust this proposal to be presented to the Authority, or a court-appointed resolution process, as seriously as the builder's proposal" (7-pt Likert, new item, full wording and validation notes in Supplementary §S5, since no off-the-shelf scale exists for this construct, per rigor item #9).
...
- Constraint comprehension (pre/post multiple-choice quiz about the specific site's building bylaws).
...
1. **No live status-quo comment-only baseline arm**, mitigated by the recalled-baseline item (§0.1), but the causal "vs. commenting" claim needs a follow-up field deployment embedded in a real, live redevelopment stakeholder process (BoundarEase-style) to fully establish.
2. **Single-session, ~2.5-hour exposure**, no data on sustained multi-session civic process use (per Orca's own flagged limitation); follow-up needs a longitudinal deployment across a multi-meeting redevelopment stakeholder process.
...
4. **Two-site, single-authority (Noida) scope**, building-bylaw generalization to other jurisdictions (other Indian cities, or a US city such as San Diego through CLARO) is unevaluated; name the specific constraint-graph-portability engineering work a multi-jurisdiction follow-up would require.
